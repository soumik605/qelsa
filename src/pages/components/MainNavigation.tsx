...
const mainNavigationItems: NavigationItem[] = [
  { id: 'profile', label: 'My Space', icon: Home, isMain: true },
  { id: 'home', label: 'Feed', icon: Activity, isMain: true },
  { id: 'qelsa-ai', label: 'Qelsa AI', icon: Zap, isMain: true },
  { id: 'jobs', label: 'Jobs', icon: Briefcase, badge: 12, isMain: true },
  { id: 'pages', label: 'Pages', icon: FileText, isMain: true },
  { id: 'connections', label: 'Network', icon: Users, isMain: true },
  { id: 'courses', label: 'Courses', icon: BookOpen, badge: 3, isMain: true },
  { id: 'blog', label: 'Blog', icon: Rss, isMain: true }
];

export function MainNavigation({ activeSection, onSectionChange, onJobSearch, onJobFilter, onProfileClick }: MainNavigationProps) {
  ...
  return (
    <>
      ...
      <header className={`hidden lg:flex items-center justify-between px-6 py-4 transition-all duration-300 sticky top-0 z-50 ${
        isScrolled 
          ? 'glass-strong backdrop-blur-xl' 
          : 'glass backdrop-blur-lg'
      }`}>
        {/* Logo */}
        <div className="flex items-center gap-3 group cursor-pointer" onClick={() => onSectionChange('profile')}>
              ...
              >
                <div className="relative">
                  <Icon className={`h-6 w-6 transition-all duration-300 ${
                    isActive ? 'text-neon-cyan glow-cyan' : 'group-hover:text-neon-cyan'
                  }`} />
                  {item.badge && (
                    <Badge 
                      className="absolute -top-2 -right-2 h-5 min-w-[20px] text-xs bg-gradient-to-r from-neon-pink to-neon-purple text-white border-0 animate-pulse"
                    >
                      {item.badge}
                    </Badge>
                  )}
                ...
                <span className={`text-xs font-medium ${
                  isActive ? 'text-neon-cyan' : 'text-muted-foreground group-hover:text-white'
                }`}>
                  {item.label}
                </span>
                {isActive && (
                  <div className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 w-1 h-1 bg-neon-cyan rounded-full glow-cyan"></div>
                )}
              </button>
            );
          })}
        ...
        <div className="flex items-center gap-3">
          <Button
            variant="ghost"
            size="icon"
            className="relative glass rounded-full hover:glass-strong hover:glow-cyan transition-all duration-300"
          >
      ...
      {activeSection === 'jobs' ? (
        <header className="lg:hidden glass backdrop-blur-xl sticky top-0 z-40">
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            <button 
              onClick={onJobFilter}
              className="w-8 h-8 rounded-full glass hover:glass-strong flex items-center justify-center hover:scale-110 transition-all duration-300"
            >
          ...
          {mainNavigationItems.map((item) => {
            ...
            return (
              <button
                key={item.id}
                onClick={() => onSectionChange(item.id)}
                className={`relative flex flex-col items-center gap-1 p-2 rounded-xl transition-all duration-300 hover:scale-110 ${
                  isActive 
                    ? 'text-neon-cyan' 
              ...
              >
                <div className="relative">
                  <Icon className={`h-5 w-5 transition-all duration-300 ${
                    isActive ? 'text-neon-cyan scale-110' : 'text-muted-foreground'
                  }`} />
                  {item.badge && (
                    <Badge 
                      className="absolute -top-1 -right-1 h-3 min-w-[12px] text-[10px] bg-gradient-to-r from-neon-pink to-neon-purple text-white border-0 animate-pulse px-1"
                    >
                      {item.badge > 9 ? '9+' : item.badge}
                    </Badge>
                  )}
                </div>
                <span className={`text-[10px] font-medium transition-all duration-300 leading-tight ${
                  isActive ? 'text-neon-cyan' : 'text-muted-foreground'
                }`}>
                  {item.label}
                </span>
                {isActive && (
                  <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-4 h-0.5 bg-neon-cyan rounded-full glow-cyan"></div>
                )}
              </button>
            );
          })}
