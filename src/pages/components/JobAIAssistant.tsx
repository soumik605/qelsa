...
export function JobAIAssistant({ 
  ...
  const placeholders = {
    compare: [
      `Compare ${filteredJobs.slice(0, 3).map(j => j.company).join(', ')}...`,
    ...
    skills: [
      ...
      'Show me the skill gaps for the UI/UX position...',
    ...
    similar: [
      'Find similar roles with better pay...',
      'Show me related positions at top companies...',
      'Recommend jobs based on my skills...',
      'What other roles match my profile?'
    ]
  };
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="relative">
            ...
            <Button
              ...
              className="absolute right-1 top-1/2 -translate-y-1/2 h-8 w-8 p-0 gradient-animated"
            >
              {isAnalyzing ? (
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>Analyzing {filteredJobs.length} job{filteredJobs.length !== 1 ? 's' : ''} from your filters</span>
          </div>
        </div>
      </Card>
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {results.map((result) => (
          <Card 
            key={result.job.id} 
            className="p-5 glass border-glass-border hover:border-neon-purple/40 transition-all duration-300 cursor-pointer"
            onClick={() => onJobClick?.(result.job)}
          >
            {/* Job Header */}
            <div className="flex items-start gap-3 mb-4">
              {result.job.companyLogo && (
                <img 
                  src={result.job.companyLogo} 
                  alt={result.job.company}
...
function SkillGapResults({ results }: { results: SkillGapResult[] }) {
  ...
  const getPriorityBg = (priority: string) => {
    switch (priority) {
      case 'high': return 'bg-neon-pink/20 border-neon-pink/30';
      case 'medium': return 'bg-neon-yellow/20 border-neon-yellow/30';
      case 'low': return 'bg-neon-green/20 border-neon-green/30';
            ...
            <div className="flex items-start justify-between mb-3">
              <div>
                ...
                <Badge className={`text-xs ${getPriorityBg(skill.priority)} border-0`}>
                  <span className={getPriorityColor(skill.priority)}>
                ...
                <div className="space-y-2">
                  {skill.learningResources.map((resource, ridx) => (
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      className="w-full justify-start text-xs border-glass-border hover:border-neon-cyan/40 hover:bg-neon-cyan/10"
                    >
        ...
        {results.map((category, idx) => (
          <Card key={idx} className="p-5 glass border-glass-border">
            <div className="flex items-center gap-2 mb-4">
              ...
              <Badge className="ml-auto text-xs bg-neon-purple/20 text-neon-purple border-0">
            ...
            <div className="space-y-3">
              {category.questions.map((question, qidx) => (
                <div 
                  key={qidx} 
                  className="p-3 rounded-lg bg-white/5 border border-glass-border hover:border-neon-cyan/30 transition-all duration-300"
                >
            ...
            <div className="flex flex-wrap gap-1.5 mb-4">
              {job.skills.slice(0, 3).map((skill) => (
                <Badge 
                  key={skill} 
                  variant="secondary" 
                  className="text-xs px-2 py-0.5 bg-white/5 border border-white/10"
                >
                  {skill}
                </Badge>
              ))}
