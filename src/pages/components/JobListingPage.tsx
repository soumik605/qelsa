...
export function JobListingPage({ 
  ...
  const jobs: Job[] = [
    {
      ...
      // Additional fields for JobDetailPage
      description: 'We are looking for a passionate Senior Frontend Developer to join our dynamic team. You will be responsible for building the next generation of our web applications using cutting-edge technologies.',
      responsibilities: [
      ...
      companyDescription: 'TechCorp is a leading technology company focused on building innovative solutions that transform how businesses operate. We pride ourselves on our collaborative culture and commitment to cutting-edge technology.',
    ...
    },
    {
      ...
      // Additional fields for JobDetailPage
      description: 'Join our fast-growing startup as a Full Stack Engineer! We are building the next generation of productivity tools and need talented engineers who can work across the entire stack.',
      responsibilities: [
      ...
      companyDescription: 'StartupXYZ is a remote-first company building innovative productivity tools for modern teams. We believe in work-life balance and provide a supportive environment for professional growth.',
    ...
    },
    {
      ...
      // Additional fields for JobDetailPage
      description: 'We are looking for a talented UI/UX Designer to join our creative team. You will be responsible for creating intuitive and beautiful user experiences for our clients digital products.',
      responsibilities: [
      ...
      companyDescription: 'DesignStudio is a boutique design agency specializing in user experience design for startups and established brands. We create meaningful digital experiences that drive business results.',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-background">
              ...
              <TabsList className="glass-strong border-glass-border">
                ...
                <TabsTrigger value="my-jobs" className="data-[state=active]:bg-neon-purple/20 data-[state=active]:text-neon-purple relative">
                  ...
                  {(savedJobs.length > 0 || viewedJobs.length > 0) && (
                    <Badge 
                      variant="outline" 
                      className="ml-2 h-5 min-w-[20px] px-1.5 bg-neon-yellow/20 text-neon-yellow border-neon-yellow/30 text-xs"
                    >
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-3 mb-3">
                          {job.companyLogo && (
                            <img 
                              src={job.companyLogo} 
                              alt={job.company}
                              className="w-10 h-10 rounded-lg object-cover border border-glass-border flex-shrink-0"
                            />
                          )}
                        ...
                        <div className="flex items-center justify-between">
                          <Badge variant="secondary" className="text-xs px-2.5 py-1 rounded-full">{job.workType}</Badge>
                          {job.fitScore && (
                    ...
                    <div className="flex items-start gap-3 mb-4">
                      {job.companyLogo && (
                        <div className="flex-shrink-0">
                          <img 
                            src={job.companyLogo} 
                            alt={job.company}
                            className="w-12 h-12 rounded-xl object-cover border border-glass-border shadow-sm"
                          />
                        </div>
                      )}
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-base line-clamp-2 mb-1 group-hover:text-neon-cyan transition-colors">
                          {job.title}
                        </h3>
                      ...
                      <div className="flex flex-col gap-1 flex-shrink-0">
                        {onToggleCompare && (
                          ...
                          >
                            {comparedJobs.some(j => j.id === job.id) ? (
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {job.skills.slice(0, 2).map((skill) => (
                        <Badge 
                          key={skill} 
                          variant="secondary" 
                          className="text-xs px-2.5 py-1 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 transition-colors"
                        >
                          {skill}
                        </Badge>
                      ))}
      ...
      {/* Compare Jobs Tray */}
      {onCompare && onRemoveFromCompare && onClearCompare && (
        <CompareJobsTray
          jobs={comparedJobs}
          onRemoveJob={onRemoveFromCompare}
          onCompare={onCompare}
