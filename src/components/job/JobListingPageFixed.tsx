import { Bookmark, BookmarkCheck, BrainCircuit, Briefcase, Building2, Clock, Eye, Filter, Globe, Home, MapPin, Search, Send, Sparkles, Target } from "lucide-react";
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-muted-foreground mt-1">Discover your next career move with AI-powered insights</p>
            </div>

            <div className="flex items-center gap-3">
              <Button variant="outline" onClick={() => setShowAIAssistant(!showAIAssistant)} className="border-neon-cyan/30 text-neon-cyan hover:bg-neon-cyan/10">
            ...
            {showAIAssistant && (
              <Card className="p-4 glass border-glass-border">
                ...
                <div className="space-y-3">
                  <Textarea placeholder="Ask me anything about these jobs..." value={question} onChange={(e) => setQuestion(e.target.value)} className="glass border-glass-border text-sm" rows={3} />
                  <Button size="sm" className="w-full bg-neon-purple hover:bg-neon-purple/90 text-white">
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-start gap-3 flex-1 min-w-0">
                        {job.companyLogo && (
                          <div className="relative flex-shrink-0">
                            <img src={job.companyLogo} alt={job.company} className="w-12 h-12 rounded-xl object-cover border border-glass-border" />
                          </div>
                        )}
                        <div className="flex-1 min-w-0">
                          <h3 className="font-semibold text-lg line-clamp-2 hover:text-neon-cyan transition-colors mb-1">{job.title}</h3>
                      ...
                      <div className="flex flex-col gap-2 items-end flex-shrink-0">
                        <div className="flex items-center gap-1">
                          ...
                          >
                            <input type="checkbox" checked={selectedJobs.includes(job.id)} onChange={() => {}} className="w-4 h-4" />
                          </Button>
                          <Button
                            variant="ghost"
                            size="sm"
                          ...
                          >
                            {job.isSaved ? <BookmarkCheck className="w-4 h-4 text-neon-cyan" /> : <Bookmark className="w-4 h-4" />}
                          </Button>
                        </div>
                      ...
                      <div className="grid gap-2">
                        {job.isQuickApplyAvailable ? (
                          <div className="grid grid-cols-2 gap-2">
                            ...
                            <Button variant="outline" size="sm" className="border-neon-cyan/30 text-neon-cyan hover:bg-neon-cyan/10 font-medium">
                              View Details
                            </Button>
                          </div>
                        ) : (
                          <Button variant="outline" size="sm" className="border-neon-cyan/30 text-neon-cyan hover:bg-neon-cyan/10 font-medium w-full">
                            View Details
                          </Button>
                        )}
                      </div>
