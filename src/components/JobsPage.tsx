...
import { 
  Search, Filter, MapPin, Building, DollarSign, Clock, Sparkles, 
  TrendingUp, Users, Brain, Star, ExternalLink, Target, Briefcase,
  ChevronRight, Plus, Calendar, Eye, Bookmark, Zap, Award, Globe,
  BarChart3, Lightbulb, ArrowRight, CheckCircle, Play, X, Send,
  BookmarkCheck
} from 'lucide-react';
...
export function JobsPage({ searchQuery, filters, aiJobResults, onSearchChange, onFilterClick }: JobsPageProps) {
  ...
  const getFilteredJobs = () => {
    ...
    return jobs.filter(job => {
      const matchesSearch = job.title.toLowerCase().includes(localSearchQuery.toLowerCase()) ||
  ...
  const renderJobCard = (job: ResponseData) => {
    ...
    return (
      <Card key={job.id} className="glass border-glass-border rounded-2xl p-5 transition-all duration-300 hover:glow-cyan hover:scale-[1.02] group cursor-pointer relative overflow-hidden h-full flex flex-col">
        {/* Exclusive Badge */}
        {isQelsaExclusive && (
        ...
        <div className="flex items-start gap-3 mb-3">
          <Avatar className="h-10 w-10 ring-2 ring-glass-border group-hover:ring-neon-cyan/50 transition-all flex-shrink-0">
            ...
            <AvatarFallback className="bg-gradient-to-br from-neon-cyan to-neon-purple text-black font-bold text-sm">
              {job.jobDetails?.company?.[0] || 'C'}
            </AvatarFallback>
          </Avatar>
          <div className="flex-1 min-w-0">
            <h3 className="font-bold text-white text-base line-clamp-2 group-hover:text-neon-cyan transition-colors mb-1">
              {job.title}
            </h3>
        ...
        <div className="flex gap-1 mt-auto">
          {!isApplied ? (
            ...
            <Button
              size="sm"
              variant="outline"
              className="flex-1 glass border-neon-green text-neon-green cursor-default text-xs h-8"
              disabled
            >
          ...
          <Button
            size="sm"
            variant="outline"
            className={`px-2 glass border-glass-border transition-all h-8 ${
              isSaved 
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-8">
        ...
        <div className="text-center space-y-4">
          <h1 className="text-4xl font-bold bg-gradient-to-r from-neon-cyan via-neon-purple to-neon-pink bg-clip-text text-transparent">
            Qelsa Jobs – Future of Work
          </h1>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            AI-powered job discovery tailored to your career journey
          </p>
        </div>
      ...
      <JobFitModal
        ...
        onClose={() => setJobFitModal({ isOpen: false, jobId: '', jobTitle: '' })}
